const commonHead = (title, bg) => `
    <head>
        ...
        <style>
            ...
            /* CAJA DE DESPLIEGUE - Capa superior garantizada con z-index alto */
            #results-dropdown { position: absolute; top: 110%; left: 0; width: 100%; background: rgba(15, 23, 42, 0.95); backdrop-filter: blur(30px); border-radius: 20px; overflow: hidden; display: none; border: 1px solid rgba(255,255,255,0.2); z-index: 9999; box-shadow: 0 20px 40px rgba(0,0,0,0.7); }
            .result-item { display: flex; align-items: center; gap: 15px; padding: 12px; text-decoration: none; color: white; border-bottom: 1px solid rgba(255,255,255,0.05); transition: 0.2s; }
            ...
            .anime-card { 
                ...
                transition: transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275), box-shadow 0.4s, opacity 0.4s;
...
const renderCards = (animes) => {
    if(!animes || animes.length === 0) return `<div style="color:rgba(255,255,255,0.5); padding: 20px 8%;">No se encontraron resultados. Intenta de nuevo más tarde o verifica el nombre.</div>`;
    return animes.map(a => {
...
const commonScript = `
    <script>
        ...
        // SCRIPT DEL BUSCADOR TOTALMENTE SANEADO
        if (input && dropdown) {
                    ...
                    if (data && data.length > 0) {
                        dropdown.innerHTML = data.map(a => \`
                            <a href="/anime/\${a.mal_id}" class="result-item">
                                <img src="\${a.images.jpg.small_image_url || a.images.jpg.image_url}" style="width:35px; height:50px; border-radius:5px; object-fit:cover;">
                                <div style="font-size:0.9rem; font-weight:600; color:white; white-space:nowrap; text-overflow:ellipsis; overflow:hidden;">\${a.title}</div>
                            </a>
                        \`).join('');
...
module.exports = {
    ...
    renderSearchResults: (q, results) => `<!DOCTYPE html><html>${commonHead("Resultados", "https://images.alphacoders.com/605/605592.png")}<body><div class="bg-hero" style="filter:brightness(0.2) blur(10px);"></div><div style="padding:40px 8%;"><a href="/" style="color:var(--main); text-decoration:none; font-weight:bold;">⬅ VOLVER</a><div class="search-wrapper" style="margin-top:20px; margin-bottom: 20px;"><div class="search-box"><span>🔍</span><input type="text" id="searchInput" placeholder="Buscar otro anime..." autocomplete="off"></div><div id="results-dropdown"></div></div><h2 class="section-title" style="padding:10px 0;">Resultados para "${q}"</h2><div class="grid-results" style="padding:0;">${renderCards(results)}</div></div>${commonScript}</body></html>`,
